...
test("projects scenes from events and skips failures", () => {
  const events: StoryEvent[] = [
    ...
    { type: "image", name: "Night", prompt: { Subject: "Night" }, error: "safety filter" },
    { type: "dialogue", replace: true, index: 1, background: "Cafe", speaker: "Mimi", caption: "Tea" },
  ...
  expect(state.scenes).toEqual([{ type: "dialogue", background: "Cafe", speaker: "Mimi", caption: "Tea", event: 4 }]);
  expect(state.assets).toEqual([{ type: "image", name: "Cafe", prompt: { Subject: "Cafe" } }]);
...
test("cards lead the log unless a user asked for one", () => {
  ...
  expect(ordered.map((event) => (event.type === "message" ? event.text : event.type === "card" || event.type === "image" ? event.name : ""))).toEqual([
    "Mimi",
    "Cafe",
    "Make a card for Yoyo",
    "Yoyo",
  ]);
...
test("events point at the scene they still own", () => {
  ...
  expect(project(events).scenes).toEqual([{ type: "dialogue", background: "Cafe", caption: "Tea", event: 2 }]);
});
