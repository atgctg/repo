...
export function leadCards(events: StoryEvent[]): StoryEvent[] {
  ...
  if (cards.length === 0 || head.slice(0, cards.length).every((event) => event.type === "card")) return events;
  const rest = head.filter((event) => event.type !== "card");
  const tail = cut < 0 ? [] : events.slice(cut);
  return [...cards, ...rest, ...tail];
}

export function project(events: StoryEvent[]): { scenes: Scene[]; assets: Asset[]; cards: Card[] } {
  const scenes: Scene[] = [];
  const assets: Asset[] = [];
  const cards: Card[] = [];
  ...
  for (let index = 0; index < events.length; index++) {
    ...
    switch (event.type) {
      ...
      case "image": {
        ...
        placeAt(scenes, { type: "image", name: event.name, event: index }, event.index, event.replace);
        break;
      }
      case "video": {
        const asset: Asset = {
        ...
        upsertAsset(assets, asset);
        placeAt(scenes, { type: "video", name: event.name, event: index }, event.index, event.replace);
        break;
      }
      case "dialogue": {
        const scene: DialogueScene = { type: "dialogue", background: event.background, event: index };
...
export function formatRanges(indices: number[]): string {
  const sorted = [...new Set(indices)].filter((index) => Number.isInteger(index)).sort((a, b) => a - b);
...
function upsertAsset(assets: Asset[], asset: Asset): void {
  const index = assets.findIndex((item) => item.name.toLowerCase() === asset.name.toLowerCase());
  if (index >= 0) assets[index] = asset;
  else assets.push(asset);
}

function upsertCard(cards: Card[], event: CardEvent): void {
  const index = cards.findIndex((card) => card.name.toLowerCase() === event.name.toLowerCase());
