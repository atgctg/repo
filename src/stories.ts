...
import { STORIES_DIR, assetDiskPath, assetUrl, safeStoryId, speechFileName, storyAssetPath } from "./files";
...
export function changeStory<T>(storyId: string, fn: (story: Story) => T | Promise<T>): Promise<T> {
...
export function getStoryCover(story: Story): string | undefined {
  for (const scene of story.scenes) {
    ...
    if (!name) continue;
    const found = story.assets.find((asset) => asset.name.toLowerCase() === name.toLowerCase());
...
function parseEvent(value: unknown): StoryEvent | undefined {
  ...
  switch (value.type) {
    case "message":
      return typeof value.text === "string" ? { type: "message", text: value.text, ...shared } : undefined;
    case "image":
      return typeof value.name === "string" ? { type: "image", ...media(value), ...insert(value), ...shared } : undefined;
    case "video":
      return typeof value.name === "string"
        ? { type: "video", ...media(value), ...frames(value), ...insert(value), ...shared }
        : undefined;
    case "dialogue": {
    ...
    case "card":
      ...
      return {
        type: "card",
        name: value.name,
        ...(value.cover === null || typeof value.cover === "string" ? { cover: value.cover } : {}),
        ...(value.voice === null || typeof value.voice === "string" ? { voice: value.voice } : {}),
        ...(isRecord(value.attributes) ? { attributes: value.attributes } : {}),
        ...shared,
      };
    case "delete": {
      if (!Array.isArray(value.indices)) return undefined;
      return { type: "delete", indices: value.indices.filter((index): index is number => typeof index === "number"), ...shared };
    }
    default:
      return undefined;
  }
...
function media(value: Record<string, unknown>): { name: string; prompt?: Record<string, unknown>; width?: number; height?: number; dominantColor?: string; references?: string[] } {
  ...
  return {
    ...
    ...(typeof value.dominantColor === "string" ? { dominantColor: value.dominantColor } : {}),
    ...(references.length > 0 ? { references } : {}),
  };
}

function frames(value: Record<string, unknown>): { firstFrame?: string; lastFrame?: string; duration?: number } {
  return {
...
async function attachFiles(story: Story): Promise<void> {
  story.assets = await Promise.all(
    story.assets.map(async (asset) => {
      const exists = await Bun.file(assetDiskPath(story.id, asset.name, asset.type)).exists();
      return exists ? { ...asset, url: assetUrl(story.id, asset.name, asset.type) } : asset;
    }),
  );
  for (const scene of story.scenes) {
    if (scene.type !== "dialogue" || !scene.caption || !scene.speaker) continue;
    const voice = story.cards.find((card) => card.name.toLowerCase() === scene.speaker?.toLowerCase())?.voice;
    if (!voice) continue;
...
export async function generateImage(
  ...
  if (!asset || asset.type !== "image" || !asset.prompt || Object.keys(asset.prompt).length === 0) {
    throw new Error(`No prompt for image "${params.name}"`);
  }
  try {
    const url = await generateStoryImage(story, asset.name, asset.prompt, asset.references ?? []);
    asset.url = url;
    return { story, asset, url };
  } catch (error) {
    console.error("img gen error", { storyId, name: params.name, error: errorMessage(error) });
    throw error;
  }
}
...
export async function generateVideo(
  ...
  if (Object.keys(prompt).length === 0 && !asset.firstFrame) throw new Error(`No prompt for video "${params.name}"`);
  try {
    ...
    console.error("video gen error", { storyId, name: params.name, error: errorMessage(error) });
    throw error;
  }
}

export function readSceneLines(story: Story, options: { offset?: number; limit?: number; last?: number } = {}): string {
  const len = story.scenes.length;
  const start =
    options.last !== undefined
      ? Math.max(0, len - options.last)
      : Math.max(0, options.offset !== undefined && options.offset < 0 ? len + options.offset : (options.offset ?? 0));
...
function formatScene(scene: Scene, index: number, story: Story): string {
  switch (scene.type) {
    ...
    case "dialogue":
      return [`[${index}] dialogue ${scene.background}`, scene.speaker, scene.caption].filter(Boolean).join("\n");
    case "video": {
      const prompt = story.assets.find((asset) => asset.name.toLowerCase() === scene.name.toLowerCase())?.prompt;
      const body = prompt ? stringify(prompt, { indent: 2 }).trim() : "";
      return body ? `[${index}] video ${scene.name}\n${body}` : `[${index}] video ${scene.name}`;
    }
    default: {
      const _exhaustive: never = scene;
      return _exhaustive;
